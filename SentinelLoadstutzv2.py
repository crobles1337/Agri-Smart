...
import traceback
import pandas as pd
from sentinelloadutils import *

logger = logging.getLogger('sentinelloader')

class Sentinel2Loaderv2:

    def __init__(self, dataPath, user, password, apiUrl='https://scihub.copernicus.eu/apihub/', showProgressbars=True, dateToleranceDays=5, cloudCoverage=(0,80), deriveResolutions=True, cacheApiCalls=True, cacheTilesData=True, loglevel=logging.DEBUG, nirBand='B08', savepath = ''):
    ...
    def getProductBandTiles(self, geoPolygon, bandName, resolution, dateReference, bpath):
        ...
        for index, sp in products_df.loc[selectedTiles].iterrows():
            ...
            if True == True:
                ...
                "deleted og downloadfile, but this is the new path"
                #downloadFile(url, tmp_tile_filejp2, self.user, self.password)
                
                
                if count ==0:
                    downloadFile(url, self.savepath + '%s%s%s.jp2' % (date1, sp['uuid'], bandName) , self.user, self.password)
                    fpath = self.savepath + '%s%s%s.jp2' % (date1, sp['uuid'], bandName)
                    #downloadFile(url, bpath, self.user, self.password)
                count = 1
            ...
            filename = open(downloadFilename, "w")
